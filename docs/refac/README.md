...
| 11 | `11-demanda-nota-fiscal-impostos.md` | `feat/nota-fiscal-impostos` | 9 (NF profissional + impostos em Configurações; escopo a definir) |
...
| 7 Comercial | ✅ | ✅ | ✅ | ✅ | ✅ | — **CONCLUÍDA (PR #22 mergeado, prod).** Cadastro de cliente em paridade (CPF/CNPJ via `br_documents` + endereço estruturado `0021_client_address` + ViaCEP). Cancelamento de venda íntegro: `PATCH→CANCELADA` bloqueado (400); ação "Cancelar venda" (`POST /vendas/{id}/cancelar`) delega ao motor `cancelar_fatura` (estorna cadeia toda; `mark_sale_cancelled` evita auto-bloqueio do motor). "Ver notas fiscais" via `GET /faturas?sale_id`. Inadimplência = avisar (não bloqueia). **+cleanup:** `soft_delete_sale` só permite excluir venda já `CANCELADA` (senão 409). Inadimplência decorativa→aviso. |
| 8 Front geral | ✅ | ✅ | ✅ | ✅ | ✅ | — **CONCLUÍDA (commits `acd1c2d`/`8150ede`, prod).** 8 listas migradas p/ `DataTable` server-side `Page[T]` (Comercial Clientes/Vendas, Compras Ordens/Cotações/Fornecedores, Estoque Itens/Recebimentos, Folha Funcionários). **Blindagem de seletores:** cada list-service ganhou helper array (`fetchPaginated page_size=100 → .items`) + versão `*Paginated` — Faturamento/PCP/Configurações (fora de escopo) não quebraram. `below_minimum` movido p/ SQL (paginação correta). Faturamento→D9 e PCP (cards D5) FORA. Dívida D6 quitada (`GET /fornecedores`→`Page[T]`). Sem migration. |
