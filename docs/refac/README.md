...
| 7 Front geral | ☐ | ☐ | ☐ | ☐ | ☐ | — Retrofit de listas legadas p/ `DataTable` server-side. **Item herdado da D6:** migrar `GET /fornecedores` para `Page[T]` (hoje `skip/limit`+`SuccessResponse`; `FornecedoresTable` pagina no cliente). |
